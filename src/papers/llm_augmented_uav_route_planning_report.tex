...
\begin{table}[H]
\centering
\caption{Example LLM sparse prediction before refinement. Values are representative dummy values for report illustration.}
...
\begin{table}[H]
\centering
\caption{Example refined path segment produced from the sparse prediction in Table~\ref{tab:llm-example}. Values are representative dummy values.}
...
\begin{table}[H]
\centering
\caption{Dummy verification result table for report completion. Replace with measured logs when final experiments are available.}
...
\begin{table}[H]
\centering
\caption{Example failed verification payload. Values are representative dummy values.}
...
\begin{table}[H]
\centering
\caption{Example passed verification payload. Values are representative dummy values.}
...
The follower uses the configured \texttt{/llm\_vision/plan\_verified} topic, \texttt{0.5 m/s} nominal speed, \texttt{0.1 m} waypoint acceptance radius, and \texttt{0.75 m} start acceptance radius. Before latching, it verifies that the first waypoint is close enough to the current vehicle pose. Each adjacent refined-waypoint pair is converted into a cubic Bezier segment with velocity feedforward, reducing abrupt setpoint changes compared to direct waypoint stepping.

Hardware demonstration videos should be linked here:

\begin{center}
\textbf{Google Drive hardware demonstration videos:} \url{https://drive.google.com/drive/folders/REPLACE_WITH_HARDWARE_DEMO_LINK}
\end{center}
...
\section{Experimental Results and Placeholders}

The final report package should include sparse waypoint visualizations, refined trajectory plots, verification metrics, hardware flight demonstrations, and execution logs. The current codebase already provides plot output through the visualizer node, with the configured output path \texttt{/tmp/llm\_vision\_plot.png}.

\begin{figure}[H]
\centering
\fbox{\begin{minipage}[c][2.0in][c]{0.86\textwidth}
\centering
Placeholder for sparse LLM waypoint plot.
\end{minipage}}
\caption{Sparse waypoint sequence generated by the LLM.}
\label{fig:sparse-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][2.0in][c]{0.86\textwidth}
\centering
Placeholder for refined trajectory overlay with obstacles.
\end{minipage}}
\caption{Refined trajectory after interpolation, nudging, and workspace clamping.}
\label{fig:refined-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][2.0in][c]{0.86\textwidth}
\centering
Placeholder for hardware execution trace or flight screenshot.
\end{minipage}}
\caption{Hardware execution trace on the ModalAI Starling 2 platform.}
\label{fig:hardware-placeholder}
\end{figure}
